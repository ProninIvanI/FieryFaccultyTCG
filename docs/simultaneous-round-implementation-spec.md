...
### 10.1. Зафиксированные проблемы после первого ручного smoke

После первого прогона в двух клиентах отдельно зафиксированы следующие follow-up проблемы:

- post-round state обновляется не полностью:
  - карта может остаться в руке после розыгрыша;
  - не всегда виден корректный переход в сброс или в persistent-объект поля;
  - восстановление ресурсов и добор нового раунда не доведены до ожидаемого поведения;
- resolved result недостаточно хорошо отражается публично:
  - после общего `lock-in` оба клиента должны видеть один и тот же уже применённый результат;
  - hidden draft должен оставаться скрытым только до `roundResolved`, а не дольше;
- отсутствует пошаговый playback резолва:
  - игрок не видит, какое действие исполняется прямо сейчас;
  - не подсвечиваются source/target и порядок применения эффектов;
- текущий ribbon/UI ещё содержит transitional визуальные проблемы:
  - нечитаемые строки и проблемы кодировок;
  - дубли effect/summary/details;
  - растягивание карточек и смешение debug-слоя с пользовательским представлением;
- финальный UX таргетинга ещё не доведён:
  - язычки и target indicators должны существовать даже при единственной допустимой цели;
  - текущая цель должна считываться с карточки и с поля без догадок.

### 10.2. Текущий практический next step

1. довести `game-core` до полного post-round lifecycle:
   - zone transitions;
   - resource refresh;
   - draw/new-round setup;
   - корректный итоговый `GameState`;
2. проверить и исправить server/public snapshot после `roundResolved`, чтобы оба клиента видели один и тот же resolved state;
3. реализовать frontend playback поверх `roundResolved.orderedActions` вместо мгновенного немого переключения state;
4. после этого зачистить ribbon-card presentation, текстовые label и target indicators;
5. затем расширять battle log и позже возвращаться к replay/debug на уже подготовленном derived-контракте.
