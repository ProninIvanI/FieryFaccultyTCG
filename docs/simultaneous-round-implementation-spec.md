...
- `frontend` отвечает только за локальный draft:
  - выбор карты/действия;
  - выбор цели;
  - reorder очереди;
  - `lock-in`;
...
Вместо этого frontend локально редактирует draft и отправляет целиком `roundDraft.replace`.
...
1. игрок собирает локальную `action queue`;
2. reorder происходит локально;
3. frontend отправляет `roundDraft.replace` целиком;
4. по кнопке `lock-in` отправляет `roundDraft.lock`;
5. пока нет резолва, UI показывает только:
   - свою очередь;
   - свой locked state;
   - готовность соперника;
6. после `roundResolved` UI показывает фактический порядок резолва и outcome.
...
- собрать горизонтальную `action queue`;
- добавить reorder/remove/lock UX;
- показать слой резолва на каждой карточке действия;
...
- игрок может добавить действие в очередь;
- игрок может reorder очередь локально;
- lock disables дальнейшее редактирование;
- UI не показывает draft соперника;
- UI различает `порядок выбора` и `слой резолва`;
